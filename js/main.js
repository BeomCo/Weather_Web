$(document).ready(function(){
    ...
    //// 배열 데이터에서 push(새데이터) : 마지막 인덱스 자리에 추가
    // city.push("Seoul"); => city = ["Seoul"];
    // city.push("Busan"); => city = ["Seoul", "Busan"];

    //// 배열 데이터에서 unshift(새데이터) : 첫번째 인덱스 자리에 추가
    ...
    function w_info(){
        ...
        // $("#weather li").empty();   //#weather ul 하위의 모든 요소들을 비워라

        //배열 데이터의 갯수를 기준으로 w_box 라는 리스트 항목을 넣어준다.
    ...
    HTTPS(HyperText Transfer Protocol over Secure Socket Layer) : 문자 데이터 주소에 이동시키겠다는 의미 + 보안 추가 (예시, norton security)
    */
